...
def _resolve_runtime_dotenv_paths(*, here: Path) -> tuple[Optional[Path], Optional[Path]]:
    """Return dotenv path to load and dotenv path to persist UI prefs.

    In frozen bundles, prefer a writable user-level file (`~/.nps-lens/.env`) bootstrapped
    from bundled `.env.example`.
    """
    ...
    # 1) Explicit override by runtime/user.
    ...
    # 2) Standard dotenv discovery from current working directory.
    discovered = find_dotenv(usecwd=True)
    if discovered:
        discovered_path = Path(discovered)
        return discovered_path, discovered_path

    # 3) Frozen builds: bootstrap a writable ~/.nps-lens/.env using bundled .env.example.
    if getattr(sys, "frozen", False):
    ...
    # 4) Non-frozen/dev: bootstrap repo-local .env from .env.example once.
