...
FORBIDDEN = {
    "confidence", "confidence_mean", "causal_score", "priority", "nps_points_at_risk",
    "nps_points_recoverable", "attributable_focus_cases", "total_nps_impact", "action_lane",
}
...
def test_rationale_contains_only_observed_or_reproducible_statistics() -> None:
    rank = pd.DataFrame({
        "nps_topic": ["Pagos > SPEI", "Acceso > Login"], "corr": [-0.71, 0.12],
        "best_lag_weeks": [1, 4], "max_cp_stability": [0.73, 0.20],
...
def test_rates_are_weighted_by_responses_and_sparse_inputs_are_omitted() -> None:
    frame = pd.DataFrame({
        ...
        "nps_topic": ["A"] * 4,
        "responses": [10, 90, 10, 90], "focus_rate": [0.0, 1.0, 0.0, 0.5],
        "nps_mean": [10.0, 8.0, 4.0, 6.0], "incidents": [0, 0, 10, 10],
    })
