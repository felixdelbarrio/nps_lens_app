...
- **Sumario del Periodo**: KPIs y gráficos afectados solo por Service + Period.
...
- **Incidencias ↔ NPS**: comparte `Canal` y `Grupo Score`; añade el método causal exclusivo de la sección.
...
El canal selecciona `Web` por defecto si existe; si no, `Todos`. El grupo selecciona `Detractores` si existe; si no, `Todos`. Los enlaces Helix se resuelven con `Record ID` y no con el número de incidencia.
