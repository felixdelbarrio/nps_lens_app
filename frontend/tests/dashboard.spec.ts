...
test("uploads a schema-drift file and shows cumulative results", async ({ page }) => {
  ...
  await expect(page.getByTestId("status-copy")).toContainText("Carga completed", {
    timeout: 180000
  });
  await expect(page.getByTestId("uploads-table")).toContainText(
  ...
  await expect(page.getByTestId("status-copy")).toContainText("Agregados recalculados.");
});
